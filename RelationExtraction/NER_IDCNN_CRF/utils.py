...
def clean(params):
    ...
    """
    configFile = 'configfiles/' + params.modelSavePath + "/"
    if os.path.isfile(params.vocab_file):
        os.remove(params.vocab_file)

    if os.path.isfile(params.map_file + '/mapfiles/' + params.modelSavePath):
        os.remove(params.map_file + '/mapfiles/' + params.modelSavePath)

    if os.path.isdir(params.ckpt_path + '/' + params.modelSavePath):
        shutil.rmtree(params.ckpt_path + '/' + params.modelSavePath)

    if os.path.isdir(params.summary_path):
        shutil.rmtree(params.summary_path)

    if os.path.isdir(params.result_path):# + '/' + params.modelSavePath
    ...
    if os.path.isfile(configFile + params.config_file):
        os.remove(configFile + params.config_file)
...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger, modelTpye):
    # create model, reuse parameters if exists
    model = Model_class(config, modelTpye)
