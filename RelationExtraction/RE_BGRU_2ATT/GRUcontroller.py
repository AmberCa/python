...
import utils2 as us
# from utils import singleModel, moreModel
import getGRUResult
...
def relationExtract(entity):
	...
	if len(per_list) > 1:
		per_list2 = us.singleModel(per_list)
		print('--------%s'%per_list2)
		# modelResultList = getGRUResult.getRelation('per_per', per_list2, sentence)
		result_list = getModelResult('per_per', per_list2, sentence, result_list)
	if len(phone_list) >= 1 and len(per_list) >= 1:
		data_list = us.moreModel(per_list, phone_list)
		print('--------%s'%data_list)
		# getGRUResult.getRelation('per_phone', data_list, sentence)
		result_list = getModelResult('per_phone', data_list, sentence, result_list)

	return result_list


def getModelResult(modelType, modelData, sentence, result_list):
	modelResultList = []
	modelResultList = getGRUResult.getRelation(modelType, modelData, sentence)
	result_list = us.copyList(modelResultList, result_list)
	return result_list
