...
import os
import network
import utils2 as us2
import relationConfig as rc
...
def getRelation(model_path, dataList, sentenceInfo):
    # print('-----------'+model_path)
    # print(os.getcwd())

    relation = rc.RelationConfig.getInstance()
    print('RelationConfig------------%s'%relation)

    pathname = us2.getModelPath2(model_path)
    print('-----------'+pathname)

    # print('rc.RelationConfig.test------%s'%rc.test)
    # wordembedding = np.load('RE_BGRU_2ATT/data/' + model_path + '/vec.npy')
    wordembedding = rc.RelationConfig.initData[model_path + 'wordembedding']
    test_settings = network.Settings()
    test_settings.vocab_size = 16693
    #test_settings.num_classes = 12
    test_settings.num_classes = us2.getClassNum(model_path)
    print('-----------'+ str(test_settings.num_classes))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            word2id = rc.RelationConfig.initData['word2id']
            ...
            # relation2id = {}
            # id2relation = {}
            relation2id = rc.RelationConfig.initData[model_path + 'relation2id']
            id2relation = rc.RelationConfig.initData[model_path + 'id2relation']
            # f = open('RE_BGRU_2ATT/origin_data/' + model_path + '/relation2id.txt', 'r', encoding='utf-8')
            ...
            result_list= []
            for pairEntity in dataList:
                sentence = sentenceInfo.strip()
                en1 = pairEntity[0]
                en2 = pairEntity[1]
                print("实体1: " + en1)
                print("实体2: " + en2)
                ...
                for n, rel_id in enumerate(top3_id):
                    if n == 0:
                        result_list.append({"entity":en1 + " " + en2, "relation":id2relation[rel_id], 
                            "probability":str(prob[rel_id])})
