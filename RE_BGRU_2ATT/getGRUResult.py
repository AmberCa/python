...
def getRelation(model_path, dataList, sentenceInfo):
    print('-----------'+model_path)
    pathname = utils.getModelPath(model_path)
    print('-----------'+pathname)
    wordembedding = np.load('./data/' + model_path + '/vec.npy')
    test_settings = network.Settings()
    test_settings.vocab_size = 16693
    #test_settings.num_classes = 12
    test_settings.num_classes = utils.getClassNum(model_path)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            relation2id = {}
            id2relation = {}
            f = open('./origin_data/' + model_path + '/relation2id.txt', 'r', encoding='utf-8')
