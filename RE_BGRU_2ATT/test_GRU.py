...
flags.DEFINE_string("model_path",    "per_per",      "Path to save model")
...
def main_for_evaluation():
    pathname = "./model/' + FLAGS.model_path + '/ATT_GRU_model"
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for model_iter in testlist:
                ...
                test_word = np.load('./data/' + FLAGS.model_path + '/testall_word.npy')
                test_pos1 = np.load('./data/' + FLAGS.model_path + '/testall_pos1.npy')
                test_pos2 = np.load('./data/' + FLAGS.model_path + '/testall_pos2.npy')
                allprob = []
                acc = []
                ...
                allans = np.load('./data/' + FLAGS.model_path + '/allans.npy')
...
def main(_):
    ...
    #pathname = "./model/' + FLAGS.model_path + '/ATT_GRU_model"
    #pathname = "./model/per_per/ATT_GRU_model"
    pathname = utils.getModelPath(FLAGS.model_path)
    print('-----------'+pathname)
    wordembedding = np.load('./data/' + FLAGS.model_path + '/vec.npy')
    #wordembedding = np.load('./data/per_per/vec.npy')
    test_settings = network.Settings()
    test_settings.vocab_size = 16693
    #test_settings.num_classes = 12
    test_settings.num_classes = utils.getClassNum(FLAGS.model_path)
